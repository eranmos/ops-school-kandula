...
export DOCKER_TLS_VERIFY=1' > /etc/profile.d/docker.sh"
  ...
  echo " =>   DOCKER_HOST=tcp://$DOCKER_HOST:4243"
  echo " =>   DOCKER_TLS_VERIFY=1"
fi

OPTIONS="--tlsverify --tlscacert=/etc/docker/ssl/ca.pem --tlscert=/etc/docker/ssl/server-cert.pem --tlskey=/etc/docker/ssl/server-key.pem -H=0.0.0.0:4243"
